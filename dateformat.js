...
// settings
window.ROAM_OVERRIDES = {
  dateformat: 'EEE Month Dth YYYY', // see for details: https://github.com/thesved/ViktoRoam/blob/master/js/dateformat.js
}

window.RoamDateFormatter?.stop === 'function' && window.RoamDateFormatter.stop()
// if (
//   window.RoamDateFormatter &&
//   typeof window.RoamDateFormatter.stop === 'function'
// ) {
//   window.RoamDateFormatter.stop()
// }

window.RoamDateFormatter = (function () {
  var dateformat =
    (window.ROAM_OVERRIDES && window.ROAM_OVERRIDES.dateformat) ||
    'YYYY.MM.DD EEE'
  ...
  var observer,
    ...
    months = (window.ROAM_OVERRIDES && window.ROAM_OVERRIDES.nameMonths) || [
      'January',
      'February',
      'March',
      'April',
    ...
    monthsShort = (window.ROAM_OVERRIDES &&
      window.ROAM_OVERRIDES.nameMonthsShort) || [
      'Jan',
      'Feb',
      'Mar',
      'Apr',
    ...
    days = (window.ROAM_OVERRIDES && window.ROAM_OVERRIDES.nameDays) || [
      'Monday',
      'Tuesday',
      'Wednesday',
      'Thursday',
    ...
    daysShort = (window.ROAM_OVERRIDES &&
      window.ROAM_OVERRIDES.nameDaysShort) || [
      'Mon',
      'Tue',
      'Wed',
      'Thu',
  ...
  function changeDate(dom, remove) {
    ...
    if (dom.querySelectorAll)
      mutations = Array.from(
        dom.querySelectorAll(remove ? '[data-original-text]' : lookfor)
      )
    // ignore textareas
    else if (
      dom.parentNode &&
      dom.parentNode.closest &&
      dom.parentNode.closest('textarea')
    )
      return
    // add text nodes
    else if (dom.textContent) mutations.push(dom)
    ...
    mutations = mutations.filter(function (_e) {
      ...
      // get date match
      var search =
        (remove && _e.dataset && _e.dataset.originalText) || _e.textContent
      var date = (search.match(
      ...
      var nodes = Array.from({ length: result.snapshotLength }, (_, index) =>
      ...
      var changed = _e.textContent.replace(date, function (_v) {
        return (
          (_v && _v[0] == '#' ? '#' : '') +
          dateFormat(
            _v.replace(/^#/, '').replace(/(\d)(st|nd|rd|th)/g, '$1'),
            dateformat
          )
        )
      })
      ...
      if (changed != _e.textContent) {
        ...
        if (_e.parentNode && _e.parentNode.dataset)
          _e.parentNode.dataset.originalText = _e.textContent
  ...
  function dateFormat(date, format) {
    ...
    text = text.replace(/dW/g, function () {
      return Math.floor(
        Math.abs(date.getTime() - today.getTime()) / 1000 / 60 / 60 / 24 / 7
      )
    })
  ...
  function getWeekOfYear(date) {
    ...
    if (
      compareDates(lastMonday, new Date(date.getFullYear(), 11, 29, 12, 0)) >=
        0 &&
      compareDates(date, lastMonday) >= 0
    )
      return 1

    // first week of year: this Monday till Thursday, otherwise next monday
    var week1 = new Date(date.getFullYear(), 0, 1, 12, 0)
    if (getDayOfWeek(week1) <= 3) week1 = addDay(-getDayOfWeek(week1), week1)
    else week1 = addDay(7 - getDayOfWeek(week1), week1)

    // calculate the difference in weeks
    var diff = (date.getTime() - week1.getTime()) / (60 * 60 * 24 * 7 * 1000)
    if (diff < 0)
      // if diff is negative it means we have to return last year's week number
      return getWeekOfYear(new Date(date.getFullYear(), 0, 0, 12, 0))
    else return upperCeil(diff)
  }
